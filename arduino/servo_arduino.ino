...
void loop() {
    ...
    if (Serial.available()) {
        ...
        sv_pitch.writeMicroseconds(
            (int)mapfloat(float(kDefaultPitch) - cmds[0].toFloat(), 0, 180, 400, 2470));
        sv_pan.writeMicroseconds(
            (int)mapfloat(float(kDefaultPan) - cmds[1].toFloat(), 0, 180, 530, 2530));
        sv_tilt.writeMicroseconds(
            (int)mapfloat(float(kDefaultTilt) - cmds[2].toFloat(), 0, 180, 420, 2470));
    }
}
