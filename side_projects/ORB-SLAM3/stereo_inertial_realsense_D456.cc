/**
 * This file was written by Christian Brice (brice.c.67b9@m.isct.ac.jp) for use
 * in ORB-SLAM3. It is a modified version of stereo_inertial_realsense_D435i.cc
 * for the RealSense D456.
 *
 * ORB-SLAM3:
...
int main(int argc, char** argv) {
    if (argc < 3 || argc > 4) {
        cerr << endl
             << "Usage: ./stereo_inertial_realsense_D456 path_to_vocabulary "
    ...
    auto imu_callback = [&](const rs2::frame& frame) {
        ...
        if (rs2::frameset fs = frame.as<rs2::frameset>()) {
            ...
            if (m_frame.get_profile().stream_name() == "Gyro") {
                ...
                current_accel_timestamp = (m_frame.get_timestamp() + offset)
                                          * 1e-3;
                ...
                while (v_gyro_timestamp.size() > v_accel_timestamp_sync.size()) {
                    ...
                    rs2_vector interp_data =
                        interpolateMeasure(target_time, current_accel_data,
    ...
    rs2::stream_profile cam_left = pipe_profile.get_stream(RS2_STREAM_INFRARED,
                                                           1);
    rs2::stream_profile cam_right = pipe_profile.get_stream(RS2_STREAM_INFRARED,
                                                            2);
    ...
    rs2_intrinsics intrinsics_left = cam_left.as<rs2::video_stream_profile>()
                                         .get_intrinsics();
    ...
    rs2_intrinsics intrinsics_right = cam_right.as<rs2::video_stream_profile>()
                                          .get_intrinsics();
    ...
    while (!SLAM.isShutDown()) {
...

        {
            ...
            while (v_gyro_timestamp.size() > v_accel_timestamp_sync.size()) {
                ...
                rs2_vector interp_data =
                    interpolateMeasure(target_time, current_accel_data,
                                       current_accel_timestamp, prev_accel_data,
                                       prev_accel_timestamp);
        ...
        for (int i = 0; i < vGyro.size(); ++i) {
            ORB_SLAM3::IMU::Point lastPoint(vAccel[i].x, vAccel[i].y,
                                            vAccel[i].z, vGyro[i].x, vGyro[i].y,
