 ...
 * @brief TODO.
 */
const void AppMgr::SetupIncludeDxlibInit() {
    ...
    if (static_cast<float>(DesktopW) / DesktopH
    ...
    // アンチエイリアス付き図形描画の準備を行う - Prepare for drawing
    // anti-aliased shapes
    BeginAADraw();
...
void AppMgr::Main() {
    ...
    if (!hebi_error) {  // HEBIアクチュエータと接続されているときのみ - Only
        ...
        std::cin >> com;
        comtext = "COM" + com;
        if (SerialWater->open(comtext.c_str()) != 0) {
        ...
        std::cin >> com;
        comtext = "COM" + com;
        if (SerialServo->open(comtext.c_str()) != 0) {
            std::cout << "  Cannot open " << comtext << std::endl;
        }
    }
...
void AppMgr::MainThread() {
    ...
    StartButton = new Button(1300 - 60 - 340 - 60 - 330, WindowH - 200 - 100,
                             340, 100, "START", this);
    OnClick(StartButton);

    ConvertButton =
        new Button(60 + 60, WindowH - 200 - 100, 340, 100, "CONVERT", this);
    StopButton = new Button(1300 - 60 - 340, WindowH - 200 - 100, 340, 100,
                            "STOP", this);
    ...
    *shot_log << std::endl;
    ...
    while (!EndFlag && ScreenFlip() == 0 && ClearDrawScreen() == 0) {
        ...
        servo_str << camera_pos[0] << " " << camera_pos[1] << " "
                  << camera_pos[2] << "\n";
...
void AppMgr::OnClick(View* view) {
    ...
    if (view == LeftButton) {
        InputBox_Theta->SetNum(InputBox_Theta->GetNum()
                               + InputBox_Increment->GetNum()
                                     / InputBox_R->GetNum() * 180 / M_PI);
        OnClick(ConvertButton);
    }
    ...
    if (view == ShotButton) {
        ...
        *shot_log << std::endl;
        std::cout << "[SNAPSHOT]  " << dts << " | Voltage: "
                  << std::to_string(InputBox_Voltage->GetNum())
                  << " V | Current: "
                  << std::to_string(InputBox_Current->GetNum()) << " A\n";
    }
