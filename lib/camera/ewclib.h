...
//           version 2.8
// Copyright (C) 2018-2021 I.N.  All rights reserved.
...
// 2020.06.26 ver.2.6 EWC_IsCaptured(),EWC_GetImage()の仕様変更，リングバッファ機能の追加
// 2021.03.30         EWC_GUIDtoTEXT()にフォーマットを追加
// 2021.04.16         ewc_rawの追加．EWC_CnvNV12toRGB32()の追加．
// 2021.04.16         EWC_PropertyPage()のスレッド化
// 2021.04.19         EWC_GetCamera()を独立化, EWC_GetCameraName(), struct ewc_camera追加
// 2021.04.20 ver.2.7 EWC_CnvY16toRGB32(), EWC_CnvY800toRGB32()の追加
// 2021.04.25 ver.2.7b EWC_SetBuffer()時はEWC_GetImage()の第２引数を省略(またはNULL)で使用
// 2021.05.13         毎回のデータサイズを取得するためEWC_IsCaptured()の引数仕様を変更
// 2021.05.13         EWC_BufferLock(),EWC_BufferUnlock()の追加
// 2021.05.13         バッファ数が2以上のとき1枚あたりのサイズはRGB32相当とした
// 2021.05.17         EWC_GetImage()の引数仕様を変更，EWC_IsCaptured()にすべて集約
// 2021.05.21 ver.2.8 ewc_rawを廃止．EWC_RawMode()を追加．
...
#endif
HANDLE ewc_hmutex[EWC_NCAMMAX];			//Mutexのハンドル
//Mutex処理
...
int ewc_raw[EWC_NCAMMAX];	//そのまま転送するなら1 (v2.8)
...
struct ewc_struct{
	...
	HANDLE hThread;			//PropertyPageスレッドのハンドルv2.7
	int ppnum;				//PropertyPageスレッドへ渡す値v2.7
	volatile int init;
	volatile int stop;
	volatile int errcode;
	BYTE *pbuf;				//画像の保存先（リングバッファの先頭）
	BYTE *buffer;			//内部で確保したリングバッファの先頭
	double ftime;			//フレーム周期(s)の実測値（最新値）
	int w_count;			//writeしたカウンタ値v2.6
	int r_count;			//readしたカウンタ値v2.6
	int buffer_r;			//バッファの選択(read)v2.6
	int buffer_w;			//バッファの選択(write)v2.6
	double buf_stime[EWC_BUF_NUM];	//各バッファのサンプル時刻(s)v2.6
	int buf_count[EWC_BUF_NUM];		//各バッファのカウンタ値v2.6
	long bufsize[EWC_BUF_NUM];		//得られた各サンプルのバイト数v2.8
	long bufsize32;					//RGB32時のバイト数（このサイズでリングバッファを確保）v2.8
	BYTE *pbufadr[EWC_BUF_NUM];		//各サンプルの先頭アドレスv2.8
...
public:
	...
	//フレームの到着毎に呼ばれるコールバック関数
	STDMETHODIMP BufferCB(double dblSampleTime, BYTE *pBuffer, long lBufferSize)
	{
		ewc_mget(i);
		//buffer_w, buffer_rの更新
		if(ewc_s[i].w_count==ewc_s[i].r_count){
			ewc_s[i].buffer_r= next_index(ewc_s[i].buffer_r);
			ewc_s[i].buffer_w= next_index(ewc_s[i].buffer_w);
		}else{
			ewc_s[i].buffer_w= next_index(ewc_s[i].buffer_w);
			if(ewc_s[i].buffer_w==ewc_s[i].buffer_r){
				ewc_s[i].buffer_r= next_index(ewc_s[i].buffer_r);
			}
		}
		int buf_w= ewc_s[i].buffer_w;		//書き込むリングバッファ番号
		BYTE *p0= ewc_s[i].pbuf + ewc_s[i].bufsize32 * buf_w;
		if(ewc_raw[i]){
			//そのままコピー(rawモード)
			memcpy(p0, pBuffer, lBufferSize);
		}else{
			//画像の上下を逆にしてコピー(RGB24,RGB32)
			int wx= ewc_s[i].wx;
			int wy= ewc_s[i].wy;
			int xbyte= lBufferSize/wy;	//１ラインのバイト数
			BYTE *src= pBuffer;
			BYTE *dst= p0 +xbyte*(wy-1);
			for(int y=0; y<wy; y++){
				memcpy(dst, src, xbyte);
				src += xbyte;
				dst -= xbyte;
			}
		}
		ewc_s[i].pbufadr[buf_w]= p0;											//データ先頭アドレス
		ewc_s[i].bufsize[buf_w]= lBufferSize;									//データサイズを格納
		ewc_s[i].ftime= dblSampleTime -ewc_s[i].buf_stime[prev_index(buf_w)];	//前回との時刻差を格納
		ewc_s[i].buf_stime[buf_w]= dblSampleTime;								//今回の時刻を格納
		ewc_s[i].buf_count[buf_w]= ++ewc_s[i].w_count;							//フレーム番号の更新と格納
		ewc_mrls(i);
		return S_OK;
	}
	//コンストラクタ	
	ewc_SampleGrabberCB(int num)
	{
		...
		ewc_s[i].buffer_w= 0;
		ewc_s[i].buffer_r= 0;
		for(int i=0; i<EWC_BUF_NUM; i++){
			ewc_s[i].buf_stime[i]= 0.;
			ewc_s[i].buf_count[i]= 0;
			ewc_s[i].pbufadr[i]= NULL;
			ewc_s[i].bufsize[i]= 0;
		}
	}
	//デストラクタ
...
//バッファのロック解除とリングバッファ管理情報の更新 v2.8
int EWC_BufferUnlock(int num)
{
	if(numCheck(num)) return 1;
	int buf_r= ewc_s[num].buffer_r;
	ewc_s[num].r_count= ewc_s[num].buf_count[buf_r];
	if(buf_r != ewc_s[num].buffer_w) ewc_s[num].buffer_r= next_index(buf_r);
	ewc_mrls(num);
	return 0;
}
...
//読み出すべき画像のサイズ(単位:バイト)を返す v2.8
//EWC_IsCaptured()でもサイズは分かる
int EWC_GetBufferSize(int num)
{
	if(numCheck(num)) return 0;
	ewc_mget(num);
	int buf_r= ewc_s[num].buffer_r;
	int size= ewc_s[num].bufsize[buf_r];
	ewc_mrls(num);
	return size;
}
...
// 新しい画像が到着したかどうか v2.8
//  num:カメラ番号
//  buffer:読み出すべき画像が格納されているアドレス（オプション）
//  size:画像データのサイズ[単位：バイト]（オプション）
//  frame:フレーム番号（オプション）
//  buffer_r:読み出すリングバッファの番号(0～)（オプション）
//  stime:取得時刻(秒)（オプション）
...
int EWC_IsCaptured(int num, BYTE **buffer=NULL, int *size=NULL, int *frame=NULL, int *buffer_r=NULL, double *stime=NULL)
{
	...
	ewc_mget(num);
	int buf_r= ewc_s[num].buffer_r;
	int dif= ewc_s[num].buf_count[buf_r] -ewc_s[num].r_count;
	if(buffer) *buffer= ewc_s[num].pbufadr[buf_r];
	if(size) *size= ewc_s[num].bufsize[buf_r];
	if(frame) *frame= ewc_s[num].buf_count[buf_r];
	if(buffer_r) *buffer_r= ewc_s[num].buffer_r;
	if(stime) *stime= ewc_s[num].buf_stime[buf_r];
	ewc_mrls(num);
	return dif;
}

//カメラ(番号:num)の画像取得 v2.8
// num:カメラ番号  buffer:画像データを格納するアドレス(NULL時は転送を省略)
// 備考：EWC_SetBuffer()を使っているときは，この関数は使わない
int EWC_GetImage(int num, void *buffer)
{
	...
	int buf_r= ewc_s[num].buffer_r;
	if(buffer){
		void *src= (void *)ewc_s[num].pbufadr[buf_r];
		memcpy(buffer, src, ewc_s[num].bufsize[buf_r]);
	}
	ewc_s[num].r_count= ewc_s[num].buf_count[buf_r];
	if(buf_r != ewc_s[num].buffer_w) ewc_s[num].buffer_r= next_index(buf_r);
	ewc_mrls(num);
...
int EWC_OneShot(int num)
{
	...
	while((c2= ewc_s[num].w_count) == c1) Sleep(1);	//１フレーム捨てる
...
//プロパティページを表示させる（従来版）~v2.6
int EWC_PropertyPage1(int num)
{
	if(numCheck(num)) return 1;
	...
	OleCreatePropertyFrame(
		HWND_DESKTOP,		// Parent window(=NULL)
		0,0,				// Reserved
...
//プロパティページ用スレッド関数 v2.7
DWORD WINAPI ewc_PropertyPage_th(LPVOID lpParam)
{
	int num= *(int *)lpParam;
	int r= EWC_PropertyPage1(num);
	return r;
}

//プロパティページを表示させる(スレッド実行) v2.7
int EWC_PropertyPage(int num)
{
	if(numCheck(num)) return 1;
	if(ewc_s[num].ppnum!=-1){	//過去に開かれた形跡あり
		//スレッドは終了しているか
		DWORD status;
		if(GetExitCodeThread(ewc_s[num].hThread, &status)){
			if(status == STILL_ACTIVE){
				return 2;	//現在開かれている
			}else{
				CloseHandle(ewc_s[num].hThread);
				ewc_s[num].hThread= NULL;
				ewc_s[num].ppnum= -1;
			}
		}
	}
	...
	ewc_s[num].ppnum= num;
	ewc_s[num].hThread= CreateThread(NULL,0,ewc_PropertyPage_th,&ewc_s[num].ppnum,0,NULL);
	if(ewc_s[num].hThread==NULL){
		ewc_s[num].ppnum= -1;
		return 3;
	}
	return 0;
...
#include <wmsdkidl.h>
#define EWC_MST_MAX 32
const struct {
	GUID guid;
	const char *s_mst;
} ewc_guid_mst[EWC_MST_MAX]= {
	...
	{MEDIASUBTYPE_P010,  "P010"},
	{MEDIASUBTYPE_P016,  "P016"},
	{MEDIASUBTYPE_P210,  "P210"},
	{MEDIASUBTYPE_P216,  "P216"},
	{MEDIASUBTYPE_GREY,  "GREY"},
	{MEDIASUBTYPE_Y8,    "Y8"},
	{MEDIASUBTYPE_Y800,  "Y800"},
	{MEDIASUBTYPE_Y16,   "Y16"},
	{MEDIASUBTYPE_BY8,   "BY8"}
};

//メディアサブタイプのGUID値に対応する文字列を生成 v2.7
int EWC_GUIDtoTEXT(GUID guid, char *s, int size)
{
	for(int i=0; i<EWC_MST_MAX; i++){
		if(guid==ewc_guid_mst[i].guid){
			strcpy_s(s, size, ewc_guid_mst[i].s_mst);
			return 0;
		}
	}
	char ss[32];
...
cont:
	...
	//リングバッファの確保，コールバック関数の登録
	//（buffer32, buffer, ewc_pSampleGrabberCB[]の設定）
	//リングバッファは常にRGB32のサイズで，枚数分，確保する v2.8
	//（他の小さいサイズのフォーマットで使用する場合，無駄が出るが，MJPGのように毎回変化するものに対応するため）
	ewc_s[num].bufsize32= sizeof(int) * ewc_s[num].wx * ewc_s[num].wy;
	ewc_s[num].buffer= new BYTE[ ewc_s[num].bufsize32 * EWC_BUF_NUM ];
...
int EWC_Close(int num)
{
	...
	if(ewc_s[num].ppnum!=-1){	//過去に開かれた形跡あり
		...
		if(GetExitCodeThread(ewc_s[num].hThread, &status)){
			if(status == STILL_ACTIVE){
				//現在開かれている
				TerminateThread(ewc_s[num].hThread, 0);
				WaitForSingleObject(ewc_s[num].hThread, 1000);
			}
			CloseHandle(ewc_s[num].hThread);
			ewc_s[num].hThread= NULL;
			ewc_s[num].ppnum= -1;
...
#define ewc_YV2R(Y,V)    ((298*(Y-16)              +409*(V-128) +128)>>8)
#define ewc_YUV2G(Y,U,V) ((298*(Y-16) -100*(U-127) -208*(V-128) +128)>>8)
#define ewc_YU2B(Y,U)    ((298*(Y-16) +516*(U-127)              +128)>>8)

// NV12フォーマットをRGB32フォーマットへ変換 v2.7
// rgb:格納先  yuv:元画像  wx,wy:幅，高さ[pixel]
void EWC_CnvNV12toRGB32(void* rgb, void* yuv, int wx, int wy)
{
    unsigned char* pUV= (unsigned char*)yuv +wx*wy;
	unsigned char* pY= (unsigned char*)yuv;
    unsigned char Y,U,V;
    ...
    for(int y=0; y<wy/2; y++){
		for(int n=0; n<2; n++){
			for(int x=0; x<wx/2; x++){
				...
				V= *pUV++;
				Y= *pY++;
				*pRGB++ = Limit(0, ewc_YU2B(Y,U), 255);
				*pRGB++ = Limit(0, ewc_YUV2G(Y,U,V), 255);
				*pRGB++ = Limit(0, ewc_YV2R(Y,V), 255);
				*pRGB++ = 0;
				Y = *pY++;
				*pRGB++ = Limit(0, ewc_YU2B(Y,U), 255);
				*pRGB++ = Limit(0, ewc_YUV2G(Y,U,V), 255);
...
// Y16フォーマットをRGB32フォーマットへ変換 v2.7
// （ただし，16ビットのうち上位8ビットのみ使う）
// rgb: 格納先  y16:元画像  wx,wy:幅，高さ[pixel]
void EWC_CnvY16toRGB32(void* rgb, void* y16, int wx, int wy)
{
	unsigned short* pY= (unsigned short*)y16;
    unsigned char Y;
	unsigned char* pRGB= (unsigned char*)rgb;
...
// Y800フォーマットをRGB32フォーマットへ変換 v2.7
// rgb: 格納先  y800:元画像  wx,wy:幅，高さ[pixel]
void EWC_CnvY800toRGB32(void* rgb, void* y800, int wx, int wy)
{
	unsigned char* pY= (unsigned char*)y800;
    unsigned char Y;
	unsigned char* pRGB= (unsigned char*)rgb;
...
// カメラの名前と数を取得する（独立版）（EWC_Open()前でも可） v2.7
int EWC_GetCameraName(ewc_camera *ewcc, int *nmax)
{
	ICreateDevEnum *pDevEnum= 0;
	IEnumMoniker *pEnum= 0;
	IMoniker *pMoniker= 0;
	int ewc_cominit_cn= 0;
	int errcode= 0;
	int n= 0;		//ビデオキャプチャデバイスのカウント用
	
	//COMの初期化
	if(!ewc_cominit){
		ewc_hr= CoInitializeEx(NULL,COINIT_MULTITHREADED);
		if(ewc_hr==S_OK) ewc_cominit_cn= 1;	//成功したらフラグを立てる
	}

	//システムデバイス列挙子の作成
	ewc_hr= CoCreateInstance(CLSID_SystemDeviceEnum,0,CLSCTX_INPROC_SERVER,IID_ICreateDevEnum,(void **)&pDevEnum);
	if(ewc_hr!=S_OK){errcode=1; goto fin_cn;}

	//列挙子の取得
	ewc_hr= pDevEnum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory,&pEnum,0);
	if(ewc_hr!=S_OK){errcode=0; goto fin_cn;}
	...
	for(int i=0; i<EWC_NCAMMAX; i++){
		if(pEnum->Next(1,&pMoniker,&cFetched)==S_OK){
			...
			//DisplayNameの取得
			LPOLESTR strMonikerName;
			ewc_hr= pMoniker->GetDisplayName(NULL,NULL,&strMonikerName);
			if(ewc_hr!=S_OK){errcode=3; goto fin_cn;}

			//DisplayNameに'@device:pnp'または'@device:sw'がある
			if(wcsstr(strMonikerName,L"@device:pnp") || wcsstr(strMonikerName,L"@device:sw")){
				if(ewcc){
					IPropertyBag *pBag= 0;
					pMoniker->BindToStorage(0,0,IID_IPropertyBag,(void **)&pBag);
					VARIANT var;
					...
					// FriendlyNameの取得
					pBag->Read(L"FriendlyName",&var,0);
					WideCharToMultiByte(CP_ACP,0,var.bstrVal,-1,ewcc[n].FriendlyName,sizeof(ewcc[n].FriendlyName),0,0);
					VariantClear(&var);

					// Descriptionの取得
					pBag->Read(L"Description",&var,0);
					WideCharToMultiByte(CP_ACP,0,var.bstrVal,-1,ewcc[n].Description,sizeof(ewcc[n].Description),0,0);
					VariantClear(&var);

					// DevicePathの取得
					pBag->Read(L"DevicePath",&var,0);
					WideCharToMultiByte(CP_ACP,0,var.bstrVal,-1,ewcc[n].DevicePath,sizeof(ewcc[n].DevicePath),0,0);
					VariantClear(&var);

					ewc_release(pBag);
				}
				//ewccがNULLのときはカメラ数を数えるだけ
				n++;
			}
			ewc_release(pMoniker);
		}
		if(ewcc) if(n >= *nmax) break;
	}
...
// カメラ台数を返す（独立版）（EWC_Open()前でも可） v2.7
int EWC_GetCamera(void)
{
	int n;
	EWC_GetCameraName(NULL,&n);
	return n;
}

//カメラ毎にRawモードを使うかを選択する v2.8
//rawmode= 0:上下反転あり（通常）　1:rawモード（そのまま転送）
